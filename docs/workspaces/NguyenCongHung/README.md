...
| K1+K2+K3 (+M3/S3) | Tổng hợp | Vận hành tổng kho (collaboration): nhập kho → xuất/điều chuyển → thu hồi, **nhúng luôn khối M3↔S3** để nhìn toàn cảnh | `bpmn-kho-van-hanh-k1-k2-k3.svg` (3 pool kho K1/K2/K3 xếp dọc + khối collaboration M3↔S3 2 pool thật nhúng ở đáy, nối bằng message flow theo mạch tồn) | Xong bản nháp — gộp K1/K2/K3 và nhúng nguyên `bpmn-kho-van-hanh-m3-s3.svg` để quan sát tổng thể |

> Lưu ý mã: file BPMN dùng hậu tố khác với mã workspace. Ánh xạ: **M3 và S3 (kiểm kê) đã gộp chung** vào sơ đồ collaboration `bpmn-kho-van-hanh-m3-s3.svg` (2 pool: M3 pool trên, S3 pool dưới, nối bằng message flow); S4 (kích hoạt tài khoản) ↔ `...-s3`, S1 (tuyển dụng) ↔ `...-s1`. **Cụm kho vận** dùng tiền tố **K**: K1 (nhập kho & QC) ↔ `...-nhap-kho-qc-k1`, K2 (xuất kho & điều chuyển) ↔ `...-xuat-kho-dieu-chuyen-k2`, K3 (thu hồi & xử lý hàng trả) ↔ `...-thu-hoi-hang-tra-k3`. Cả ba quy trình kho khớp mạch tồn với M3/S3 qua message flow (K1 cấp tồn đầu vào; K2 nhận đầu ra phân bổ của M3; K3 cập nhật lại tồn/hao hụt về S3/M3). Ngoài ba sơ đồ đơn lẻ (giữ nguyên), có thêm **sơ đồ gộp collaboration `bpmn-kho-van-hanh-k1-k2-k3.svg`**: ba pool thật (K1 trên, K2 giữa, K3 dưới) trên một canvas, nối bằng message flow toàn cục (K1 cross-dock → K2; K2 hàng lỗi/hàng trả → K3), cùng phong cách với `bpmn-kho-van-hanh-m3-s3.svg`. **Sơ đồ gộp này còn nhúng nguyên khối collaboration M3↔S3 (2 pool thật) ở đáy canvas** để nhìn toàn cảnh năm quy trình trên cùng một hình: cả ba kho đồng bộ tồn/hoạch định với M3/S3 qua message flow (K1 cấp tồn đầu vào; K2 nhận lệnh phân bổ của M3; K3 báo hao hụt/cập nhật tồn về S3/M3). Cần thống nhất lại bảng ánh xạ khi gộp vào báo cáo chung.
