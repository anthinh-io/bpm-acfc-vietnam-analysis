# BÁO CÁO CÁ NHÂN – QUY TRÌNH M3, S3 & S4 (ACFC)
...
> **Mục đích tài liệu:** đây là **báo cáo tổng hợp cấp cá nhân** cho ba quy trình Hưng phụ trách — **M3 (Quản lý)**, **S3 (Hỗ trợ)** và **S4 (Hỗ trợ)** — gộp liền mạch discovery, mô tả hiện trạng (AS-IS), bộ câu hỏi thu thập dữ liệu, phân tích định tính/định lượng và đề xuất TO-BE. Tài liệu tự chứa (self-contained): mọi bảng bước, kịch bản, câu hỏi phỏng vấn và bảng KPI đều nằm trong file này để có thể copy/paste và định dạng lại thành báo cáo Word khi nhóm chắt lọc. Số liệu định lượng còn thiếu tuân thủ nguyên tắc **không tự bịa số ACFC** — xem mục 9 và 11.
...
Phạm vi cá nhân của Hưng thuộc mảng **hàng hóa, kho, tồn kho và tài khoản thành viên ACFC**. Theo yêu cầu rút gọn của nhóm (cập nhật 11/08/2026), mỗi thành viên bắt buộc hoàn thiện **1 quy trình Quản lý + 1 quy trình Hỗ trợ** (nhóm đã đạt tối thiểu 6 quy trình BPMN + phân tích ở cấp chung). Hưng dồn toàn lực hoàn thiện **M3 (Quản lý)** và **S3 (Hỗ trợ)** theo yêu cầu bắt buộc, đồng thời bổ sung **S4 (Hỗ trợ thứ hai)** — quy trình đăng ký/kích hoạt tài khoản thành viên khai thác từ nội dung công khai trên trang chủ ACFC, đã kiểm tra không trùng mã hoặc nội dung với quy trình của thành viên khác. C3/C4 (2 quy trình Cốt lõi) không còn trong phạm vi bài làm tuần này nên đã gỡ khỏi workspace cá nhân.
...
- Phạm vi tuần này chỉ gồm M3, S3 và S4; không mô tả các quy trình Quản lý/Cốt lõi/Hỗ trợ khác đã thuộc phạm vi phụ trách của thành viên khác trong nhóm.
...
| Tiêu chí rubric | Yêu cầu | Đóng góp của M3/S3/S4 trong báo cáo này |
|---|---|---|
| 1. Liệt kê & phân loại quy trình | Tối thiểu 10 quy trình, ≥3 mỗi cấp | M3 (Quản lý), S3 (Hỗ trợ), S4 (Hỗ trợ) — góp 3/10 trong phạm vi cá nhân (danh mục 10 quy trình đầy đủ ở cấp nhóm) |
| 2. Mô tả quy trình | Mô tả bằng lời: bước (Động từ + Danh từ), actor, input/output, gateway, kịch bản thành/bại | Đầy đủ ở mục 4 — bảng bước + actor + ghi chú hệ thống + kịch bản thành công/thất bại cho cả M3, S3, S4 |
| 3. Mô hình hóa BPMN | Mô hình hóa quy trình bằng BPMN 2.0 (pool/lane, gateway, nhiều End, Split & Join cùng loại) | **Đã vẽ** — M3: `diagrams/bpmn-hoach-dinh-phan-bo-hang-hoa-m3.svg`; S3: `diagrams/bpmn-kiem-ke-ton-kho-s2.svg`; S4: `diagrams/bpmn-dang-ky-kich-hoat-tai-khoan-s3.svg`. Cả ba sơ đồ đều đạt **8 cổng điều kiện** (M3: 2 cặp XOR split–merge + 1 cặp AND split–join + 2 XOR quyết định; S3: 6 XOR + 1 cặp AND; S4: 8 XOR), đầy đủ **Split & Join cùng loại**, có `.drawio` nguồn đi kèm — xem mục 3–4 |
| 4. Phân tích quy trình | ≥20 câu phỏng vấn (10 định tính + 10 định lượng); VA/BVA/NVA; 4 loại lãng phí; phân tích nguyên nhân gốc; định lượng Thời gian/Chất lượng/Chi phí có công thức + tính toán | M3/S3/S4 đều có bộ câu hỏi 10+10 (mục 6), VA/BVA/NVA + 4 lãng phí + bảng vấn đề–nguyên nhân–khắc phục (mục 8) và **phân tích định lượng đầy đủ 3 nhóm chỉ số có công thức + ví dụ tính toán** (mục 9) |
...
| Xác thực + BPMN | 14–17/08 | Đã vẽ BPMN M3/S3/S4 (`diagrams/`); gộp toàn bộ discovery + AS-IS + phân tích vào một báo cáo tổng; còn lại: phỏng vấn xác thực (nếu tiếp cận được nhân sự ACFC), cập nhật gateway/số liệu thật và TO-BE |
...
### 8.4. Bốn loại lãng phí (Move – Hold – Over-processing – Defects)
...
### 8.5. Phân tích nguyên nhân gốc (Vấn đề – Nguyên nhân – Khắc phục)
...
*Nhận xét: PCE 35% nghĩa là gần 2/3 thời gian là chờ (NVA) — khớp với lãng phí "Chờ (Hold)" ở mục 8.4.*
...
M3, S3 và S4 đã có đủ mô tả bước–actor–kịch bản thành/bại (rubric tiêu chí 2), sơ đồ BPMN (rubric tiêu chí 3 — cả M3, S3 và S4 đều đạt 8 cổng điều kiện gồm XOR và AND, có đầy đủ Split & Join cùng loại), bộ câu hỏi 10+10 mỗi quy trình và phân tích VA/BVA/NVA + 4 loại lãng phí + phân tích nguyên nhân gốc + phân tích định lượng ba nhóm chỉ số có ví dụ tính toán + đề xuất khắc phục (rubric tiêu chí 4), dựa trên bằng chứng công khai (mức A/B) và giả định cần xác thực (mức C). Khoảng trống còn lại:
...
3. **Đối chiếu mã sơ đồ nhóm:** file BPMN trong `diagrams/` dùng hậu tố nhóm (`m3`, `s2`, `s3`) khác với mã workspace (M3, S3, S4) — cần thống nhất bảng ánh xạ mã khi gộp vào báo cáo chung để tránh nhầm S3 (kiểm kê) với S4 (kích hoạt tài khoản). Ánh xạ hiện tại: S3 (kiểm kê) ↔ `...-s2`, S4 (kích hoạt tài khoản) ↔ `...-s3`.
